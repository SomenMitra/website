...
import { CitizenProfileService } from 'src/app/citizen-portal/service-api/citizen-profile.service';
...
export class SeedDbtApplyComponent implements OnInit {
 ...
 dealerArr:any[] = [];
  ...
  farmerIdValidation :number =0;
  applctnNm :number=0;
  inhctValidation:number =0;
  ...
  @ViewChild('someSeedDBTPreModal') someSeedDBTPreModalRef: ElementRef;
  docSectnSts = false; // document section display/ not
  isMobile = false;

  objRedirect: any;

  seedPreId = environment.seedDBTPre;
  seedId = environment.seedDBT;
  applicationType: any;
  availabilityAmt: any;
  availedSubsidy: any;
  availedBag: any;
  ...
  ngOnInit(): void {
    ...
    // this.schemeId = environment.seedDBT;
    this.schemeId = this.route.snapshot.paramMap.get('id');
    this.applctnId = 0;
    this.getSchmDmgCtrls();
    this.getSchmDynmCtrls();
    this.lang = localStorage.getItem('locale');
    this.getAppldSchmLst();


    setTimeout(() => {
      $('.seedApplydealerName').css("width","200px");
      $('.seed-type,.seed-variety,.clsIntendedMonth').css("width","150px");
      $('.seed-bagsize').css("width", "60px");
      $('.seed-noofbag').css("width", "50px");
      $('.seed-quantity,.seedFinalTotCost,.seed-total-cost,.payableAmtFarmer,.seedfarmerNonPaddyAmt').css("width", "100px");
  ...
  getSchmDynmCtrls() {
    ...
    this.objSchmCtrl.schemeDynCtrls(params).subscribe(res => {
      ...
      this.vchMaintainance = res.result['vchMaintainance'];
      if (res.status > 0) {
        this.getDistList();
        ...
        setTimeout(function () {
          ...
          $('.seed-dbt-farmername').attr('readonly','readonly');
          ...
          var calc = $('.clsAddMoreSeed th').length-2;
          $('.clsAddMoreSeed th:nth-child('+calc+')').hide();
          $('.seedfarmerNonPaddyAmt').closest('td').hide();
          $('.seedfarmerNonPaddyAmt').attr('readonly', 'readonly');
          var addmoreSeedElement = document.querySelector(".seedSchemeType");

          if(curObj.schemeId == environment.seedDBT){
            var container = document.createElement("div");
            container.classList.add('col-md-12','col-lg-12','text-right','mb-4');
            container.innerHTML = '<input type="button" value="Add seed details" name="btnopenSeedDetails" id="btnopenSeedDetails" class="openSeedDetails btn btn-success " data-toggle="modal" data-target="#someSeedDBTModal" (click)="openSeedDetails(1,this.vchMaintainance.isMaintainance)"> ';
           ...
           document.getElementById('btnopenSeedDetails').addEventListener("click", function() {
            ...
            curObj.openSeedDetails(1,curObj.vchMaintainance.isMaintainance)
          });
          }else{
            var container = document.createElement("div");
            container.classList.add('col-md-6','col-lg-4','mt-4');
            container.innerHTML = '<input type="button" value="Add seed details" name="btnopenSeedDetails" id="btnopenSeedDetails" class="openSeedDetails btn btn-success " data-toggle="modal" data-target="#someSeedDBTModal" (click)="openSeedDetails(2)"> ';
           addmoreSeedElement.closest('.col-lg-4').after(container);
           ...
           document.getElementById('btnopenSeedDetails').addEventListener("click", function() {
            ...
            curObj.openSeedDetails(2,curObj.vchMaintainance.isMaintainance)
          });
          }

        $('.seed_dbt_farmer_id').on('blur',function(){
          curObj.validateFarmerId();
        });
          $('.seedSchemeType ,.seed_dbt_farmer_id').on('change', function () {

          var selectedSchemeType = $(this).val();
          $(".openSeedDetails").show();
          if(curObj.schemeId == environment.seedDBT){
            $('.clsAddMoreSeed th:nth-child('+calc+')').hide();
            $('.seedfarmerNonPaddyAmt').closest('td').hide();
            $('.seedfarmerNonPaddyAmt').val(0.00);
            $('.totBkAmtdiv').addClass('d-none');
            $('.btnsubmit').text('Save & Next');
          }else{
            if(selectedSchemeType == 'Kharif (2023)')
            {
              // $('.clsIntendedMonth').html('<option value="0"> --Select-- </option>');
              curObj.getCropList('K');
            }
            else
            {
              curObj.getCropList('R');
            }

          }

          curObj.removeRetailer('','','',1);
        });
        if(curObj.schemeId != environment.seedDBT){
          $('.clsAddMoreSeed th:nth-child('+calc+')').show();
          $('.seedfarmerNonPaddyAmt').closest('td').show();
          $('.seedfarmerNonPaddyAmt').val('');
          $('.btnsubmit').text('Pay');
        }

        $('.clsType').on('change',function(){
          $('.totBkAmtdiv').removeClass('d-none');
          curObj.applicationType = $(this).val();
          $('.seed-total-cost').val('');
          $('.clsDealerName').val('');
          $('.clsDealerMobileNo').val('');

          if($(this).val() == 'Dealer'){
            // curObj.open(curObj.someSeedDBTPreModalRef);
            // curObj.fillDealerDetailsPreBook(1);
            var addDealerElement1 = document.querySelector(".clsDealerName");
            var container = document.createElement("div");
            container.classList.add('col-md-6','col-lg-4','mt-4');
            container.innerHTML = '<input type="button" value="Choose Dealer" name="btnopenDealerDetails" id="btnopenDealerDetails" class="openDealerDetails btn btn-success " data-toggle="modal" data-target="#someSeedDBTPreModal" (click)="openDealerDetails()"> ';
           addDealerElement1.closest('.col-lg-4').after(container);

           document.getElementById('btnopenDealerDetails').addEventListener("click", function() {
            let districtIdObj: any = ( < HTMLInputElement > document.getElementById('selDistrict')).value;
            if (!curObj.vldChkLst.selectDropdown(districtIdObj, "District")) {
              return false;
            }
            let blocktIdObj: any = ( < HTMLInputElement > document.getElementById('selBlock')).value;
            if (!curObj.vldChkLst.selectDropdown(blocktIdObj, "Block")) {
              return false;
            }
            curObj.open(curObj.someSeedDBTPreModalRef);
            curObj.fillDealerDetailsPreBook(1);
            ...
            $('.clsDealerName').attr('readonly', 'readonly');
            $('.clsLicenceNo').attr('readonly', 'readonly');
            $('.clsDealerMobileNo').attr('readonly', 'readonly');
            curObj.typeofApply = 2;
            $('.clsAddMoreSeed th:nth-child('+ ($('.clsAddMoreSeed th').length-3) +')').hide();
            $('.seed-total-cost').closest('td').hide();
          }else{
            var removeDealerElement1 = document.querySelector(".openDealerDetails")|| false;
            if (removeDealerElement1) {
              removeDealerElement1.parentElement.remove();
            }

            curObj.typeofApply = 1;
            $('.clsAddMoreSeed th:nth-child('+ ($('.clsAddMoreSeed th').length-3) +')').show();
            $('.seed-total-cost').closest('td').show();
          }
        });
  ...
  getCropList(seasonCode){
    this.loading = true;
    let param = {
      "seasonCode": seasonCode,
    }
    this.objSeedService.getCropList({param}).subscribe(res => {
    this.respSts = res.status;
     if(this.respSts == 200)
     {
      this.loading = false;
  ...
  getAllSeed(blockIdObj){
      let param = {
        "blockId": blockIdObj
      }

      this.loading = true;
      this.objSeedService.getAllSeed({param}).subscribe(res => {
        this.respSts = res.status;
        if(this.respSts == 200)
        {
          this.loading = false;
  ...
  getAllSeedAvailablity()
  {
    if(this.schemeId == environment.seedDBT){
      ...
      // let blockIdObj: any = ( < HTMLInputElement > document.getElementById('selBlock')).value;

      // if (!this.vldChkLst.selectDropdown(blockIdObj, "Block")) {
      //   $('.seedSchemeVarity').val(0);
      //   return false;
      // }

      // if(this.responseSeedAvailable.length > 0){
      //   this.responseSeedAvailable = this.responseSeedAvailable;
      //   if(this.applicationType == 'Dealer'){
      //     var dealer = $('.clsDealerName').val();
      //     if(dealer != ''){
      //       this.responseSeedAvailable = this.responseSeedAvailable.filter(item => item.firmName === dealer);
      //     }
      //   }
  ...
  openSeedDetails(from: any, maintainanceSts: any) {
    if (maintainanceSts == 0) {
      this.seedTotCost = '0';
      if (from == 2) {
        var dealerName = $('.clsDealerName').val();
        if (dealerName == '' && $('.clsType').val() == 'Dealer') {
          Swal.fire({
            icon: 'error',
            text: "select dealer"
          });
      ...
      let description=this.vchMaintainance.description;
      Swal.fire({
            icon: 'error',
            text: description
          });
    ...
    let seedSchemeTypeElement = < HTMLElement > document.querySelector(".seedSchemeType");
      ...
      if (selectedTypeValue == '0' || selectedTypeValue == "") {
        if(this.schemeId == environment.seedDBT){
          ...
          Swal.fire({
            icon: 'error',
            text: "Farming Season can not be left blank"
          });
        }
      ...
      if(this.schemeId == environment.seedDBT){
        ...
        for (let i = 0; i < this.typeOfSeedOptions.length; i++) {
          optCrops += '<option data-val="' + this.typeOfSeedOptions[i]['Crop_Code'] + '" value="' + this.typeOfSeedOptions[i]['CROP_NAME'] + '">' + this.typeOfSeedOptions[i]['CROP_NAME'] + '</option>';
        }

        ( < HTMLInputElement > document.getElementById('selseedSchemeCrop')).innerHTML = optCrops;

        if(this.responseSeedAvailable.length > 0 && this.typeofApply == 2){
          if(this.applicationType == 'Dealer'){
            var dealer = $('.clsDealerName').val();
            if(dealer != ''){
              this.responseSeedAvailable = this.responseSeedAvailable.filter(item => item.firmName === dealer);
        ...
        this.fillDealerDetailsPreBook(2);
      }
  ...
  fillVariety() {
    if(this.schemeId == environment.seedDBT){
      ...
      this.loading = true;
      var cropCodeValue = $('.seedSchemeCrop').find(':selected').data('val');
      var selectedSchemeType = $('.seedSchemeType').find(':selected').val();

     if(this.typeOfVarietyOptions.length > 0){
      this.loading = false;
      let optVariety = '<option value="0" selected="selected">Select</option>';
      for (let i = 0; i < this.typeOfVarietyOptions[0].length; i++) {
        if(this.typeOfVarietyOptions[0][i]['CROP_ID'] === cropCodeValue){
          optVariety += '<option data-val="' + this.typeOfVarietyOptions[0][i]['CROP_VERID'] + '" data-bagsize="' + this.typeOfVarietyOptions[0][i]['BAG_SIZE_KG'] + '" value="' + this.typeOfVarietyOptions[0][i]['Variety_Name'] + '">' + this.typeOfVarietyOptions[0][i]['Variety_Name'] + '  (' + this.typeOfVarietyOptions[0][i]['BAG_SIZE_KG'] + ' KG)</option>'
        }
      }
      ( < HTMLInputElement > document.getElementById('selseedSchemeVarity')).innerHTML = optVariety;
     }else{
        var seasonCode = '';
        if(selectedSchemeType == 'Kharif')
        {
          seasonCode = 'K';
        }else{
          seasonCode = 'R';
        ...
        this.objSeedService.getVarietyList({param}).subscribe(res => {
          this.respSts = res.status;
          if(this.respSts == 200)
          {
            this.loading = false;
            ...
            this.typeOfVarietyOptions  = Array.from(this.apiCropVarietyResult);
            this.typeOfVarietyOptions[0].sort(function (x, y) {
              let a = x.Variety_Name.toUpperCase(),
                  b = y.Variety_Name.toUpperCase();
              return a == b ? 0 : a > b ? 1 : -1;
            });
  ...
  fillDealerDetailsPreBook(from:any){
    ...
    var cropCodeValue = $('.seedSchemeCrop').find(':selected').data('val');
    var cropCodeVarietyValue = $('.seedSchemeVarity').find(':selected').data('val');
    var cropCodeVarietyBagSize = $('.seedSchemeVarity').find(':selected').data('bagsize');
    let districtIdObj: any = ( < HTMLInputElement > document.getElementById('selDistrict')).value;
    if (!this.vldChkLst.selectDropdown(districtIdObj, "District")) {
      $('.seedSchemeVarity').val(0);
      return false;
    ...
    if(from == 2){
      if(this.typeofApply == 2){
        var tableClass = '';
        let dlrCode;
        for (const element of this.responseSeedAvailable) {
          ctr += 1;

          licenceNoSeed += element['licenceNo'];
          currOBJ.dealerArr[element['licenceNo'] + cropCodeVarietyValue] = [];
          // console.log(this.responseSeedAvailable);
          for (const eachValue of this.responseSeedAvailable) {
            m += 1;
            var contactNo =  (element['phone']!='') ? element['phone'] : '--';
            currOBJ.dealerArr[element['licenceNo'] + cropCodeVarietyValue].push(eachValue);
          }

          if(element['dealer'] == 'OSSC'){
            dlrCode = '0000';
          }else{
            dlrCode = '0103';
          }

          tableClass += `<input type="button" class="btn-sm btn btn-primary m-1 clsDealer" data-dealercode="` + dlrCode + `"  data-val="` + element['licenceNo'] + `" data-bagsize="` + cropCodeVarietyBagSize + `" data-dealername="` +element['firmName']  + `" data-contactno="` + contactNo  + `" value="Select" />`;
        }
        $('.noDataTxtDanger').remove();

        let seedSchemeVarietyElement = < HTMLElement > document.querySelector(".seedSchemeVarity");
        if (Number(m) == 0) {
          container.innerHTML = "<center class='text-danger noDataTxtDanger'><b>Dealer wise data is not available for this Block</b></center>";
          seedSchemeVarietyElement.parentElement.parentElement.parentElement.after(container);
        ...
        var tableClass = '<table class="table  table-bordered dealerTable"><thead><tr><th scope="col">Sl#</th><th scope="col">Dealer Name</th><th scope="col">Mobile no.</th><th scope="col">Action</th></tr></thead><tbody>';
        let dlrCode;
        for (const element of this.responseSeedAvailable) {
          ctr += 1;
          ...
          tableClass += `<tr class="disableTr"><th scope="row">${ctr} </th><td>${element['firmName'] } </td>
          <td>${contactNo}</td>
          <td><input type="button" class="btn-sm btn btn-primary m-1 clsDealer" data-dealercode="` + dlrCode + `"  data-val="` + element['licenceNo'] + `" data-bagsize="` + cropCodeVarietyBagSize + `" data-dealername="` +element['firmName']  + `" data-contactno="` + contactNo  + `" value="Select" /></td></tr>`;
        }
      ...
      $(".clsDealer").on('click', function() {
        ...
        // var selectedSeedSchemeCropCode    = $('.seedSchemeCrop').find(':selected').data('val');
        var selectedSeedSchemevariety     =  $('.seedSchemeVarity').val();
        var selectedseedvbagSize          = $('.seedSchemeVarity option:selected').attr('data-bagsize');
        var selectedDealerName            = $(this).attr('data-dealername');
        var selectedDealerCode            = $(this).attr('data-dealercode');
        var selectedDealerMobile          = $(this).attr('data-contactno');
        var availabilityStock             = $('.seedAvailability').val();
        var seedDBTFarmerId               = $('.seed_dbt_farmer_id').val();
        var selectedSchemeType            = $('.seedSchemeType').find(':selected').val();

        if (availabilityStock > 0 ) {
          $('.openSeedDetails').hide();
        } else {
          $('.openSeedDetails').show();
        }

        currentObj.getSubsidyHectare(seedDBTFarmerId, selectedSchemeType, selectedSeedSchemeCrop);
       ...
       if(selectedSeedSchemevariety == '0')
       {
        ...
        var selectedSchemeVarietyText = $('.seedSchemeVarity').find(':selected').val();

        var selectedSchemeVarietyVal = $('.seedSchemeVarity').find(':selected').data('val');
        var selectedSchemeCropVal =   $('.seedSchemeCrop').find(':selected').data('val');

        var selectedLicenceNo = $(this).data('val');
        var arrBagList = [];
        arrBagList.push({
          "FARM_NAME" : selectedDealerName,
          "CONTACT_NUMBER" : selectedDealerMobile,
          "BAG_SIZE" : selectedseedvbagSize,
          "LICENCE_NUMBER" : selectedLicenceNo,
          "VARIETY" : selectedSchemeVarietyVal,
          "CROP" : selectedSchemeCropVal,
        });

        var resultPriceList: any = '';
        for (const eachValue of arrBagList) {
          if (typeof(currentObj.arrBags['BAG_' + selectedseedvbagSize + '_' + selectedLicenceNo + '_' + selectedSchemeVarietyVal]) == 'undefined') {
            currentObj.arrBags['BAG_' + selectedseedvbagSize + '_' + selectedLicenceNo + '_' + selectedSchemeVarietyVal] = [];
          }
          resultPriceList = currentObj.getPriceList(selectedSchemeCropVal, selectedSchemeVarietyVal ,selectedDealerCode);
          let merge = $.extend({}, eachValue, resultPriceList)
          currentObj.arrBags['BAG_' + selectedseedvbagSize + '_' + selectedLicenceNo + '_' + selectedSchemeVarietyVal].push(merge);
        }
        ...
        for (const eachValue of arrBagsKeys) {
          $('.clsDealerName').val(currentObj.arrBags[eachValue][0].FARM_NAME);
          $('.clsDealerMobileNo').val(currentObj.arrBags[eachValue][0].CONTACT_NUMBER);
          if (currentObj.arrLicenceNo.length > 0 && !currentObj.arrLicenceNo.includes(currentObj.arrBags[eachValue][0].LICENCE_NUMBER + currentObj.arrBags[eachValue][0].VARIETY)) {
            ...
            clneData.find('.seedfarmerNonPaddyAmt').addClass('text-right');
            clneData.find('.seedfarmerNonPaddyAmt').attr('readonly','readonly');
            clneData.find('.seedfarmerNonPaddyAmt').val(0);
            clneData.find('.seedfarmerNonPaddyAmt').attr('id', 'seedfarmerNonPaddyAmt_' + slNo);
            ...
            document.getElementById('removeBtn_' + slNo).addEventListener("click", function() {
              currentObj.removeRetailer('BAG_' + selectedseedvbagSize + '_' + selectedLicenceNo + '_' + selectedSchemeVarietyVal,'removeBtn_' + slNo , selectedLicenceNo + selectedSchemeVarietyVal);
            });

            document.getElementById('minusBtn_' + slNo).addEventListener("click", function() {
              currentObj.calcSeedPre("2", currentObj.arrBags, slNo, currentObj.arrBags[eachValue][0].BAG_SIZE, currentObj.arrBags[eachValue][0].LICENCE_NUMBER, currentObj.arrBags[eachValue][0].VARIETY, totalSumAva)
            });

            document.getElementById('seedNoOfBag_' + slNo).addEventListener("keyup", function() {
              currentObj.calcSeedPre("3", currentObj.arrBags, slNo, currentObj.arrBags[eachValue][0].BAG_SIZE, currentObj.arrBags[eachValue][0].LICENCE_NUMBER, currentObj.arrBags[eachValue][0].VARIETY, totalSumAva)
            });

            document.getElementById('plusBtn_' + slNo).addEventListener("click", function () {
              currentObj.calcSeedPre("1", currentObj.arrBags, slNo, currentObj.arrBags[eachValue][0].BAG_SIZE, currentObj.arrBags[eachValue][0].LICENCE_NUMBER, currentObj.arrBags[eachValue][0].VARIETY, totalSumAva)
            });
            ...
            $('.seedfarmerNonPaddyAmt').attr('readonly','readonly');
            $('.seedfarmerNonPaddyAmt').addClass('text-right');
            $('.seedfarmerNonPaddyAmt').val(0);
            ...
            document.getElementById('removeBtn_1').addEventListener("click", function() {
              currentObj.removeRetailer('BAG_' + selectedseedvbagSize + '_' + selectedLicenceNo + '_' + selectedSchemeVarietyVal,'removeBtn_1', selectedLicenceNo + selectedSchemeVarietyVal);
              $('#btnopenSeedDetails').show();
            });
            ...
            document.getElementById(calcSeedMinusId).addEventListener("click", function() {
              currentObj.calcSeedPre("2", currentObj.arrBags, 0, currentObj.arrBags[eachValue][0].BAG_SIZE, currentObj.arrBags[eachValue][0].LICENCE_NUMBER, currentObj.arrBags[eachValue][0].VARIETY, totalSumAva);
            });
            ...
            document.getElementById(noofbagElementId).addEventListener("keyup", function() {
              currentObj.calcSeedPre("3", currentObj.arrBags, 0, currentObj.arrBags[eachValue][0].BAG_SIZE, currentObj.arrBags[eachValue][0].LICENCE_NUMBER, currentObj.arrBags[eachValue][0].VARIETY, totalSumAva);
            });
            ...
            document.getElementById(calcSeedPlusId).addEventListener("click", function () {
              currentObj.calcSeedPre("1", currentObj.arrBags, 0, currentObj.arrBags[eachValue][0].BAG_SIZE, currentObj.arrBags[eachValue][0].LICENCE_NUMBER, currentObj.arrBags[eachValue][0].VARIETY, totalSumAva)
            });
          }
      ...
      let selectDealerSectionElement = < HTMLElement > document.querySelector(".selectDealerSection");
      if (Number(m) == 0) {
        container.innerHTML = "<center class='text-danger noDataTxtDanger'><b>Dealer wise data is not available for this Block</b></center>";
        selectDealerSectionElement.parentElement.parentElement.after(container);
      } else {
        container.innerHTML = tableClass;
        selectDealerSectionElement.parentElement.parentElement.after(container);
      }
      ...
      $(".clsDealer").on('click', function() {
        var selectedDealerName = $(this).attr('data-dealername');
        var selectedDealerMobile = $(this).attr('data-contactno');
        var selectedDealerLicenceNo = $(this).attr('data-val');
        let dealer_name_Element  = <HTMLElement>document.querySelector(".clsDealerName");
        let dealer_name_id = dealer_name_Element.id;
        (<HTMLInputElement>document.getElementById(dealer_name_id)).value = selectedDealerName;
        currentObj.schemeForm.patchValue({ [dealer_name_id]: selectedDealerName });


        let dealer_mobile_Element  = <HTMLElement>document.querySelector(".clsDealerMobileNo");
        let dealer_mobile_id = dealer_mobile_Element.id;
        (<HTMLInputElement>document.getElementById(dealer_mobile_id)).value = selectedDealerMobile;
        currentObj.schemeForm.patchValue({ [dealer_mobile_id]: selectedDealerMobile });

        let dealer_licence_Element  = <HTMLElement>document.querySelector(".clsLicenceNo");
        let dealer_licence_id = dealer_licence_Element.id;
        (<HTMLInputElement>document.getElementById(dealer_licence_id)).value = selectedDealerLicenceNo;
        currentObj.schemeForm.patchValue({ [dealer_licence_id]: selectedDealerLicenceNo });
  ...
  fillDealerDetails() {
    ...
    $(".clsDealer").on('click', function() {
     ...
     if(selectedSeedSchemevariety == '0')
     {
      ...
      // console.log(currentObj.arrBags);
      var arrBagsKeys: any = Object.keys(currentObj.arrBags);
      var tdata = ''
      var ctrl = 0;
      currentObj.modalService.dismissAll();
  ...
  calcSeed(btnParm, arrBagsParm, cuurSlno, bagSize, dlrLcNo, VarietyCode, totalSumAva) {
    var typeValue = '';
    if(this.schemeId == this.seedPreId) {
        let typeElement = < HTMLElement > document.querySelector(".clsType");
        typeValue = ( < HTMLInputElement > document.getElementById(typeElement.id)).value;
    }
    let seedSchemeTypeElement = < HTMLElement > document.querySelector(".seedSchemeType");
    var selectedTypeValue = ( < HTMLInputElement > document.getElementById(seedSchemeTypeElement.id)).value;
    if(cuurSlno == 0) {
      ...
      let calcSeedseedfarmerNonPaddyAmtElement = < HTMLElement > document.querySelector(".seedfarmerNonPaddyAmt");
      let calcSeedseedfarmerNonPaddyAmtId = calcSeedseedfarmerNonPaddyAmtElement.id;
      ...
      if(typeValue == 'Dealer') {
          seedTotCost = '0';
          $('.totalVoucherCost').closest('td').hide();
      } else {
          $('.totalVoucherCost').closest('td').show();
      }
      let totFnlCost = (Number(totCost) * cropQuantityData);
      let seedTotFinalCst = totFnlCost.toFixed(2);
      let calcSeedfarmerNonPaddyAmt: number = 0;
      if(this.schemeId == environment.seedDBTPre)
      {
          calcSeedfarmerNonPaddyAmt = Number(((totFnlCost * environment.seedDBTBOOKINGAMTPERCNT) / 100).toFixed(2));
      }
      let farmerPayAmt = Number(seedTotFinalCst) - (Number(seedTotCost) + calcSeedfarmerNonPaddyAmt);
      var cuurentArr = {
          ...
          'calcSeedfarmerNonPaddyAmt': calcSeedfarmerNonPaddyAmt
      }
      var arrAllresult = this.totQuantity(cuurentArr, calcSeedNoOfBagId);
      if(arrAllresult) {
          // console.log(seedTotFinalCst.toString());
          ...
          ( < HTMLInputElement > document.getElementById(calcSeedTotCostId)).value = !(isNaN(parseFloat(seedTotCost))) ? seedTotCost.toString() : '0';
          ( < HTMLInputElement > document.getElementById(calcSeedTotFinalCostId)).value = !(isNaN(parseFloat(seedTotFinalCst))) ? seedTotFinalCst.toString() : '0';
          ( < HTMLInputElement > document.getElementById(calcSeedseedfarmerNonPaddyAmtId)).value = !(isNaN(parseFloat(calcSeedfarmerNonPaddyAmt.toFixed(2)))) ? (calcSeedfarmerNonPaddyAmt.toFixed(2)).toString() : '0';
          ( < HTMLInputElement > document.getElementById(calcSeedpayableAmtFarmerCostId)).value = !(isNaN(parseFloat((farmerPayAmt).toFixed(2)))) ? ((farmerPayAmt).toFixed(2)).toString() : '0';
          ...
          $('.bookingAmtNonPaddy').text(arrAllresult['bookAmt'].toFixed(2));
          if(selectedTypeValue == 'Paddy')
          {
              $('.bookingAmtNonPaddy').closest('td').hide();
              $('.bookingAmtNonPaddy').hide();
              $('.bookingAmtNonPaddy').text(' ');
          } else if(selectedTypeValue == 'Non Paddy')
          {
              $('.bookingAmtNonPaddy').closest('td').hide();
              $('.bookingAmtNonPaddy').hide();
              $('.bookingAmtNonPaddy').text(' ');
          ...
          {
              $('.bookingAmtNonPaddy').closest('td').show();
              $('.bookingAmtNonPaddy').show();
              $('#' + calcSeedpayableAmtFarmerCostId).attr('data-qunatityinhadlwise', arrAllresult['indHactare'].toFixed(2));
              $('.totBkAmt').text(arrAllresult['bookAmt'].toFixed(2));
          }
      }
    } else {
      ...
      let calcSeedseedfarmerNonPaddyAmtId = 'seedfarmerNonPaddyAmt_' + cuurSlno;
      ...
      let seedTotFinalCst =  totFnlCost.toFixed(2);
      let calcSeedfarmerNonPaddyAmt: number = 0;
      if(selectedTypeValue == 'Non Paddy')
      {
          calcSeedfarmerNonPaddyAmt = Number(((totFnlCost * environment.seedDBTBOOKINGAMTPERCNT) / 100).toFixed(2));
      }
      ...
      if(arrAllresult) {
          ...
          ( < HTMLInputElement > document.getElementById(calcSeedseedfarmerNonPaddyAmtId)).value = (calcSeedfarmerNonPaddyAmt.toFixed(2)).toString();
  ...
  totQuantity(cuurentArr, id, btnParm = 0) {
    let arrTot = {};
    ...
    var bookAmt:number    = (Number(cuurentArr.calcSeedfarmerNonPaddyAmt) > 0) ? Number(cuurentArr.calcSeedfarmerNonPaddyAmt) : 0;
    $('.dynSeedTbdy .seed-noofbag').not('#' + id).each(function() {
      ...
      indHactare += Number($(this).parents('tr').find('.seed-quantity').attr('data-quantityinhect'));
      bookAmt += Number($(this).parents('tr').find('.seedfarmerNonPaddyAmt').val());
    });
  ...
  //calculate subsidy prebooking
  calcSeedPre(btnParm, arrBagsParm, cuurSlno, bagSize, dlrLcNo, VarietyCode, totalSumAva) {
    // var arrBagsKeys: any = Object.keys(this.arrBags);
    // if(arrBagsKeys.length > 0){
    //   for(let i = 0; i < arrBagsKeys.length; i++){
    //     console.log(arrBagsParm[arrBagsKeys[i]])
    //   }
    // }
    ...
    var selectedTypeValue = ( < HTMLInputElement > document.getElementById(seedSchemeTypeElement.id)).value;
  	...
  	if (cuurSlno == 0) {
        ...
        newBagValue = Number(noOfBagvalue) - 1;
  		...
  		var cropQuantityData: any = Number(arrBagsParm['BAG_' + bagSize + '_' + dlrLcNo + '_' + VarietyCode][0].BAG_SIZE) * newBagValue;
  		var maxQuantity = Number(arrBagsParm['BAG_' + bagSize + '_' + dlrLcNo + '_' + VarietyCode][0].MAX_SUBSIDYQTY) * 100;
  		var subsidyUpTo = Number(arrBagsParm['BAG_' + bagSize + '_' + dlrLcNo + '_' + VarietyCode][0].SP_SUBSIDYQTY);
  		var indHactare = Number(cropQuantityData) / (subsidyUpTo * 100);
  		let totSubsKg = Number(arrBagsParm['BAG_' + bagSize + '_' + dlrLcNo + '_' + VarietyCode][0].TOT_SUBSIDY) / 100;
      var cropVariertyName = $('.seed-variety').val();
      this.getAllSeedInventoryAvailablity(VarietyCode);
      if (cropQuantityData > this.availabilityAmt) {
        Swal.fire({
          icon: 'error',
          text: 'Seed Quantity Cannot Be Greater Than Available Quantity'
        });
        return false;
      }
      $('#' + calcSeedNoOfBagQuantityId).attr('data-quantityinhect', indHactare);
      $('#' + calcSeedNoOfBagQuantityId).attr('data-subsidyupto', subsidyUpTo);
      $('#' + calcSeedNoOfBagQuantityId).attr('data-cropquantitydata', cropQuantityData);
      $('#' + calcSeedNoOfBagQuantityId).attr('data-maxquantity', maxQuantity);
      $('#' + calcSeedNoOfBagQuantityId).attr('data-totsubkg', totSubsKg);
      ...
      let calcSeedseedfarmerNonPaddyAmtElement = < HTMLElement > document.querySelector(".seedfarmerNonPaddyAmt");
  		...
  		let totCost = Number(arrBagsParm['BAG_' + bagSize + '_' + dlrLcNo + '_' + VarietyCode][0].ALL_IN_COST_PRICE) / 100;
      let totSubsidyKg = '0';
      // if(!((Number(this.totQunatInHect) + Number(arrBagsParm['BAG_' + bagSize + '_' + dlrLcNo + '_' + VarietyCode][0].SP_SUBSIDYQTY)) > environment.seedDBTTOTHECTValdn)){

      if ((indHactare + Number(this.availedSubsidy)) >= environment.seedDBTTOTHECTValdn) {
        let restQntity = 0;

        let totAvailed = Math.floor(indHactare + this.availedSubsidy);

        if (totAvailed == environment.seedDBTTOTHECTValdn) {
          if (this.availedBag > 0) {
            restQntity = this.availedQnt; //12.5 * 0.4 = 5
            maxQuantity = maxQuantity - restQntity;
            if (maxQuantity > 0) {
              this.seedTotCost = ((Number(totSubsKg) * maxQuantity).toFixed(2));  // 12.5 * 150 = 1875
            } else {
              maxQuantity = 0;
            }
          } else {
            this.seedTotCost = ((Number(totSubsKg) * maxQuantity).toFixed(2));
          }
          totSubsidyKg = (maxQuantity / Number(totSubsKg)).toFixed(2);
        } else {
          totSubsidyKg = this.sub.toFixed(2);
        }
      }
      else {
        if (this.sub < environment.seedDBTTOTHECTValdn) {
          this.sub = this.sub + subsidyUpTo;
        }
        if (cropQuantityData > maxQuantity) {
          this.seedTotCost = ((Number(totSubsKg) * maxQuantity).toFixed(2));
          totSubsidyKg = (maxQuantity / Number(totSubsKg)).toFixed(2);
        } else {
          this.seedTotCost = ((Number(totSubsKg) * cropQuantityData).toFixed(2));
          totSubsidyKg = (cropQuantityData / Number(totSubsKg)).toFixed(2);
        }
      }
      // }
      ...
      if(typeValue == 'Dealer'){
        ...
        $('.totalVoucherCost').closest('td').show();
  		...
  		$('#' + calcSeedNoOfBagQuantityId).attr('data-totSubsidyKg', totSubsidyKg);
      ...
      calcSeedfarmerNonPaddyAmt  = Number(((totFnlCost*environment.seedDBTBOOKINGAMTPERCNT)/100).toFixed(2));
  		...
  		let farmerPayAmt = Number(seedTotFinalCst)  - (calcSeedfarmerNonPaddyAmt);

  		var cuurentArr = {
  			'newBagValue': newBagValue,
  			'seedTotCost': this.seedTotCost,
  			'totSubsidyKg': totSubsidyKg,
  			'seedTotFinalCst': seedTotFinalCst,
  			'cropQuantityData': cropQuantityData,
  			'farmerPayAmt': farmerPayAmt,
  			'indHactare': indHactare,
  			'totSubsKg': Number(totSubsKg),
  			'maxQuantity': maxQuantity,
        'calcSeedfarmerNonPaddyAmt' : calcSeedfarmerNonPaddyAmt
  		}

      var arrAllresult = this.totQuantity(cuurentArr, calcSeedNoOfBagId, btnParm);
      (<HTMLInputElement>document.getElementById(calcSeedTotCostId)).value = '0';
  		if (arrAllresult) {
  			( < HTMLInputElement > document.getElementById(calcSeedNoOfBagQuantityId)).value = cropQuantityData.toString();
  			( < HTMLInputElement > document.getElementById(calcSeedNoOfBagId)).value = Number(newBagValue).toString();
  			( < HTMLInputElement > document.getElementById(calcSeedTotCostId)).value = !(isNaN(parseFloat(this.seedTotCost))) ? this.seedTotCost.toString() : '0';
  			( < HTMLInputElement > document.getElementById(calcSeedTotFinalCostId)).value = !(isNaN(parseFloat(seedTotFinalCst))) ? seedTotFinalCst.toString() : '0';
        ( < HTMLInputElement > document.getElementById(calcSeedseedfarmerNonPaddyAmtId)).value = !(isNaN(parseFloat(calcSeedfarmerNonPaddyAmt.toFixed(2)))) ? (calcSeedfarmerNonPaddyAmt.toFixed(2)).toString() : '0';
  			( < HTMLInputElement > document.getElementById(calcSeedpayableAmtFarmerCostId)).value = !(isNaN(parseFloat((farmerPayAmt).toFixed(2)))) ? ((farmerPayAmt).toFixed(2)).toString() : '0';
  			$('.totalClass').removeClass('d-none');
      ...
      calcSeedfarmerNonPaddyAmt  = Number(((totFnlCost*environment.seedDBTBOOKINGAMTPERCNT)/100).toFixed(2));
  		...
  		let farmerPayAmt = Number(seedTotFinalCst) - (Number(this.seedTotCost) + calcSeedfarmerNonPaddyAmt);
  		var cuurentArr = {
  			'newBagValue': newBagValue,
  			'seedTotCost': this.seedTotCost,
  			'totSubsidyKg': totSubsidyKg,
        ...
        'calcSeedfarmerNonPaddyAmt' : calcSeedfarmerNonPaddyAmt
  		...
  		if (arrAllresult) {
  			( < HTMLInputElement > document.getElementById(calcSeedNoOfBagQuantityId)).value = cropQuantityData.toString();
  			( < HTMLInputElement > document.getElementById(calcSeedNoOfBagId)).value = Number(newBagValue).toString();
  			( < HTMLInputElement > document.getElementById(calcSeedTotCostId)).value = this.seedTotCost.toString();
  			( < HTMLInputElement > document.getElementById(calcSeedTotFinalCostId)).value = seedTotFinalCst.toString();
        ( < HTMLInputElement > document.getElementById(calcSeedseedfarmerNonPaddyAmtId)).value = (calcSeedfarmerNonPaddyAmt.toFixed(2)).toString();
  			( < HTMLInputElement > document.getElementById(calcSeedpayableAmtFarmerCostId)).value = ((farmerPayAmt).toFixed(2)).toString();
  			$('.totalClass').removeClass('d-none');
  ...
  //   var curObj = this;
  //   var indHactare = 0;
  //   $('.seed-quantity').each(function(index) {
  //     let subsidyUpTo = $(this).attr('data-subsidyupto');
  //     let cropQuantityData = $(this).attr('data-cropquantitydata');
  //     let maxQuantity = $(this).attr('data-maxquantity');
  //     let totSubsKg = $(this).attr('data-totsubkg');

  //     if (cropQuantityData > maxQuantity) {
  //       curObj.seedTotCost = ((Number(totSubsKg) * Number(maxQuantity)).toFixed(2));
  //     }else{
  //       curObj.seedTotCost = ((Number(totSubsKg) * Number(cropQuantityData)).toFixed(2));
  //     }
  //   });
  // }
  ...
  removeRetailer(arrKey,removeBtnId,arrLicencekey,status=0)
  {
    ...
    else
    {
      ...
      this.arrLicenceNo.splice(0, this.arrLicenceNo.length);
 ...
 {
  let districtIdObj: any = (<HTMLInputElement>document.getElementById('selDistrict')).value;
  districtIdObj = JSON.parse(districtIdObj).val;
  let prefixArr = environment.prefixArr;

  let seedDbtfarmerElement = < HTMLElement > document.querySelector(".seed_dbt_farmer_id");
  let seedDbtfarmerId = seedDbtfarmerElement.id;
  let txtFarmerId = (<HTMLInputElement>document.getElementById(seedDbtfarmerId)).value;

  if(txtFarmerId!='')
  {
    let districtName = prefixArr[districtIdObj].toLowerCase();
    let farmerId = txtFarmerId.slice(0, 3).toLowerCase();
    if(districtName != farmerId){
      Swal.fire({
        icon: 'error',
        text: 'Invalid farmer id'
      });
      $('.btnsubmit').hide();
      this.farmerIdValidation =1;
      return false;
    }
    let apiParam = {
    ...
    this.objProf.getRedirectFarmerAPI(apiParam).subscribe(res => {
      ...
      if (res.status == 1) {
        ...
        if(redirectFrnmName!='')
        {
          let farmer_name_Element  = <HTMLElement>document.querySelector(".seed-dbt-farmername");
          let farmer_name_id = farmer_name_Element.id;
          (<HTMLInputElement>document.getElementById(farmer_name_id)).value = redirectFrnmName;
          this.schemeForm.patchValue({ [farmer_name_id]: redirectFrnmName });

          let farmer_mobile_Element  = <HTMLElement>document.querySelector(".seed-dbt-farmermobileNo");
          let farmer_mobile_id = farmer_mobile_Element.id;
          (<HTMLInputElement>document.getElementById(farmer_mobile_id)).value = res.result.resultInfo.res['VCHMOBILENO'];
          this.schemeForm.patchValue({ [farmer_mobile_id]:  res.result.resultInfo.res['VCHMOBILENO'] });
            this.farmerIdValidation =0;
            $('.btnsubmit').show();
        }
        else{
          Swal.fire({
    ...
    if (valSts) {
      if(this.schemeId == environment.seedDBTPre){
        if(!$('#agree').is(":checked")){
          Swal.fire({
            icon: 'error',
            text: 'Please select T&C checkbox to proceed'
          });
          return false;
        }
      }

      /*if($('.totBkAmt').text() == '0.00'){
        Swal.fire({
          icon: 'error',
          text: 'Booking amount can not be zero'
        });
        return false;
      }*/
      ...
      for (let key of controlKeys) {
        ...
        if (ctrlType == 8) {
          ...
          (objAddMr.addMoreData).push(addMoreChildData);
            ...
            var seedfarmerNonPaddyAmt = $(this).find('.seedfarmerNonPaddyAmt').val();
            var clsMerchantId  = $(this).find('.clsMerchantId ').val();

            var seedDBTFarmerId    = $('.seed_dbt_farmer_id').val();
            var seedDBTFarmerName  = $('.seed-dbt-farmername').val();
            var seedDBTFarmerMno   = $('.seed-dbt-farmermobileNo ').val();
            var seedDBTAadharno   = $('.seed-dbt-aadharNo').val();
            var seedSchemeType = $('.seedSchemeType').val();
            var totBookCostt      =  $('.totBkAmt').text();
            var totQuantInHect     = $(this).find('.payableAmtFarmer').data('qunatityinhadlwise');
            var totSubsidyKg     = $(this).find('.seed-quantity').data('totsubsidykg');
            var arrAllBagData      = curObj.arrBags['BAG_'+seedbagSizevalue+'_'+dealerLicenceNo+'_'+varietyCodeValue];
            var clsIntendedMonth    = (curObj.schemeId == environment.seedDBTPre) ? $(this).find('.clsIntendedMonth').val() : '';
            var clsType    = (curObj.schemeId == environment.seedDBTPre) ? $('.clsType').val() : '';

            if(curObj.schemeId == environment.seedDBTPre){
              if(clsIntendedMonth == ''){
                valSts = false;
                Swal.fire({
                  icon: 'error',
                  text: 'Please Select Intended Month of Purchase'
                });
                return false;
              }
            }
           ...
           var arrjsnTotalDetails = {
                ...
                allBagInputData   =
            {
              ...
              'seedDBTFarmerName':seedDBTFarmerName,
              'seedDBTFarmerMno' : seedDBTFarmerMno,
              'seedDBTAadharno' : seedDBTAadharno,
              'seedSchemeType': seedSchemeType,
              'seedBookingCost' : seedfarmerNonPaddyAmt,
              'totalBookingCost' : totBookCostt,
              'clsMerchantId' : clsMerchantId,
              'clsIntendedMonth' : clsIntendedMonth,
              'clsType' : clsType,
              'totSubsidyKg' : totSubsidyKg,
            }
       ...
       if (this.schemeForm.status === 'VALID') {
         let ctrlClassarr =  (ctrlClass!='' && typeof(ctrlClass)!='undefined') ? ctrlClass.split(" "):[];
          if (!this.vldChkLst.dynCtrlVal(ctrlValParam, this.el)) {
            ...
            break;
           ;
          }if( ctrlClassarr.includes("valid_aadhar") && this.isDraft==false && !this.vldChkLst.validAadhar(elmVal) ){
            valSts = false;
            Swal.fire({
              icon: 'error',
          ...
          if (result.isConfirmed) {
            ...
            else
            {
              ...
              if(this.schemeId == environment.seedDBTPre)
              {
                ...
                xmlHttp.setRequestHeader("Access-Control-Allow-Origin","https://google.com");
                // myHeaders.set("Access-Control-Allow-Origin", "https://google.com");
                var mapForm = document.createElement("form");
                mapForm.method = "POST"; // or "post" if appropriate
                mapForm.action = res.responseData.url;
                var hdn_qs = document.createElement("input");
                hdn_qs.type = "hidden";
                hdn_qs.name = "merchIdVal";
                hdn_qs.setAttribute("value", res.responseData.merchIdVal);
                mapForm.appendChild(hdn_qs);

                var hdn_qs = document.createElement("input");
                hdn_qs.type = "hidden";
                hdn_qs.name = "EncryptTrans";
                //hdn_qs.setAttribute('Access-Control-Allow-Origin', 'http://localhost:8000');
                hdn_qs.setAttribute("value", res.responseData.encryptTrans);
                mapForm.appendChild(hdn_qs);

                document.body.appendChild(mapForm);
                mapForm.submit();
              }else{
                this.router.navigate(['/home/seed-preview', encAppCtnId]);
              }
            }
            });
  ...
  getAllSeedInventoryAvailablity(cropcode = '') {
    var cropCodeValue = (cropcode == '') ? $('.seedSchemeVarity').find(':selected').data('val') : cropcode;
    this.objSeedService.getSeedInventoryAvailablity(cropCodeValue).subscribe(res => {
      this.availabilityAmt = res.result;
      if (this.availabilityAmt >= 0) {
        $('.seedAvailability').val();
        $(".clsDealer").prop('disabled', false);
      } else {
        $(".clsDealer").prop('disabled',true);
      }
    });
  }

  getSubsidyHectare(seedDBTFarmerId, selectedSchemeType, selectedSeedSchemeCrop) {
    if (seedDBTFarmerId != '' && selectedSchemeType != '') {
      let params = {
        "seedDBTFarmerId": seedDBTFarmerId,
        "selectedSchemeType": selectedSchemeType,
        "selectedSeedSchemeCrop": selectedSeedSchemeCrop

      }
      this.objSeedService.getSubsidyHectare(params).subscribe(res => {
        if (res.result != null && res.result != '') {
          this.availedSubsidy = res.result.totalHectare;
          this.availedBag = res.result.intNoOfBag;
          this.availedQnt = res.result.availedQnt;
        } else {
          this.availedSubsidy = 0;
          this.availedBag = 0;
          this.availedQnt = 0;
        }
      },
  ...
  clearData() {
    this.availabilityAmt = '';
  }

  goToPage(pageName: string) {
    let encPageName = this.encDec.encText((pageName).toString());
    this.router.navigate(['/home/seed-dbt-demo', encPageName ]);
}
}
