import { Component, OnInit, ElementRef, Injectable, ViewChild,AfterViewInit } from '@angular/core';
...
export class SchemeApplyComponent implements OnInit,AfterViewInit {
  loading = false;
  ...
  ngOnInit(): void {
    ...
    setTimeout(function () {
      $("input[type='radio'].non_bhulekh_land_details").on("click", function () {
  ...
  getSchmDynmCtrls() {
    ...
    this.objSchmCtrl.schemeDynCtrls(params).subscribe(res => {
     ...
     // console.log( this.mkuy_sector);
      const indices = [0,1,2,3,6];
      this.mkuy_sector=indices.map(index => this.mkuy_sector[index]);
      ...
      if (res.status > 0) {
        ...
        this.modeofFinance = (schmHrDt.hasOwnProperty('vchmeanoffinance')) ? schmHrDt.vchmeanoffinance : '';
        this.workflowId = (schmHrDt.hasOwnProperty('intWorkflowEffectId')) ? schmHrDt.intWorkflowEffectId : 1;
        ...
        for (let sectnInfo in this.respDynm) {
          ...
          for (let ctrlInfo in this.respDynm[sectnInfo]['sectionCtrls']) {
            ...
            if (ctrlClass == 'type_of_applicant') {
              this.group_details = ctrlNm;
            }
            ...
            if (ctrlArr['tinControlType'] == 8) {
              ...
              //console.log(ctrlArr['jsnControlArray'][0]);
              ...
              //
              if(ctrlArr['jsnControlArray'][0].minimumTime!='undefined'){
                this.minmunCount=ctrlArr['jsnControlArray'][0].minimumTime;
              }
              else{
                this.minmunCount=0
              }
              if(ctrlArr['jsnControlArray'][0].maximumTime!='undefined'){
                this.maximumTime=ctrlArr['jsnControlArray'][0].maximumTime;
              }
              else{
                this.maximumTime=0
              }
              ...
              if (ctrlVal &&
                Object.keys(ctrlVal).length > 0 &&
                ctrlVal['addMoreData'] &&
                Array.isArray(ctrlVal['addMoreData']) &&
                ctrlVal['addMoreData'][0] !== undefined &&
                ctrlVal['addMoreData'][0].length > 0) {
  ...
  setDynamicVal() {
    ...
    dynObjKey.forEach((field: any) => {
      ...
      if (this.dynElement[field].secCtrlType == 8 && this.dynElement[field].secCtrlType!=5) {
    ...
    if (this.inter_data != '' && this.inter_data != 'undefined') {
      ...
      if(parseInt(type_project)==environment.INTER_SECTOR_TYPE){
       ...
       // $(".product").css('display','none');
        ...
        $(".milletService").css('display','none');
        $('.milletService').parent().hide();
        $("input.cinNumber[type=text]").val('0');
        $(".milletType ").val('');

      }
      else if(parseInt(type_project)==environment.MILLET_SERVICE_CENTER){
       
        $(".milletService").css('display','block');
        $('.milletService').parent().show();
      }
      else{
    ...
    if (this.intra_data != '' && this.intra_data != 'undefined') {
      ...
      let type_project = this.dynElement[this.intra_data]["val"];
       ...
       if(parseInt(type_project)==environment.INTRA_SECTOR_SECTOR){
        ...
        $(".unit").val('');
        $("select.mkuy_project").val(0);
        // let mkuyproject_field: any = document.getElementsByClassName("select.mkuy_project");
        // let mkuyproject_field_id = mkuyproject_field.getAttribute('id')
        // this.schemeForm.patchValue({ [mkuyproject_field_id]: 0 });
      }else{
    ...
    // if (this.group_details != '' && this.group_details != 'undefined') {
    //   let groupval = this.dynElement[this.group_details]["val"];
    //   if(groupval==environment.GROUP){
    //     $("#"+environment.GROUP_MEMBER).css('display','block');
    //     $("#"+environment.ENTER_PRISE_MEMBER).css('display','none');
    //   }if(groupval==environment.ENTERPRISE){
    //     $("#"+environment.GROUP_MEMBER).css('display','none');
    //     $("#"+environment.ENTER_PRISE_MEMBER).css('display','block');
    //   }if(groupval==environment.INDIVIDUAL){
    //     $("#"+environment.GROUP_MEMBER).css('display','none');
    //     $("#"+environment.ENTER_PRISE_MEMBER).css('display','none');
    //   }
    // }
  ...
  async doSchemeApply() {
    ...
    if(intMkuyProjectSector=='5' && intMkuyProjectType=='86'){
     
      let sector_details_len = $(".sector_details input").length;
      let sectorDetailsInputs = $(".sector_details input");
      if(sector_details_len>0){
        let valuesArray = []; // Initialize an empty array to hold the values
        for(let i=0;i<sector_details_len;i++){
          let sector_details_id = sectorDetailsInputs.eq(i).attr('id');
          let sector_details_value = $('#' + sector_details_id).val();
          valuesArray.push(sector_details_value);
         
        }
        const allSame = valuesArray.every(value => value === valuesArray[0]);
        if (allSame) {
          Swal.fire({
            icon: 'error',
            text:"Please change 'Inter-Sectoral Department' to 'Intra-Sectoral Department' because you have applied in the same sector."
          });
          return false;
        } 
      }
    ...
    if (valSts) {
      ...
      for (let key of controlKeys) {
       ...
       if(jsnControlArray[0].ctrlClass=='type_of_applicant'){
        ...
        if (ctrlType == 8) {
          ...
          let minimumTime=0;
          let maximumTime=0;
          let AddMorelableName=this.controlTypeArr[key]['vchLabelName'];
          if(this.controlTypeArr[key]['jsnControlArray'][0].minimumTime!='undefined' ||this.controlTypeArr[key]['jsnControlArray'][0].minimumTime!=''){
            minimumTime=this.controlTypeArr[key]['jsnControlArray'][0].minimumTime;
          }
          if(this.controlTypeArr[key]['jsnControlArray'][0].maximumTime!='undefined' ||this.controlTypeArr[key]['jsnControlArray'][0].maximumTime!=''){
           maximumTime=this.controlTypeArr[key]['jsnControlArray'][0].maximumTime;
          }
        ...
        //  console.log(adMrArr.controls.length); return;
          for (let adMrCtrlRw = 0; adMrCtrlRw < adMrArr.controls.length; ++adMrCtrlRw) {
            ...
            let obChldjAddMr = [];
           //console.log(objCtrlKey,'--->'); return false
           // console.log(key + '_' + adMrCtrlRw,'===')
            AddMoreCount++;
      ...
      if(validateMkuySector ==environment.INTRA_SECTOR_SECTOR){
        intraclassCheckFlag = true;
        
      }

      if(validateMkuySector==environment.INTERSECTOR && validateMkuyProjectSector==environment.INTER_SECTOR_TYPE){
        interclassCheckFlag = true;
          ...
          if(AddMoreCount<2 && this.isDraft==false && addmoreFlagDep==1 && this.intdirectorate==environment.APICOL_Directorate && classCheckFlag){
           ...
           if(AddMoreCount<2 && this.isDraft==false && intraclassCheckFlag && AddMorelableName=='intraSectorData' ){
            Swal.fire({
              icon: 'error',
              text: 'Minimum 2 Project Details Required'
            });
            return false;
          }

           if(AddMoreCount<2 && this.isDraft==false && interclassCheckFlag && AddMorelableName=='Sector_Details'){
            ...
            if (dispNnSts === false) {
              if(validateMkuySector ==environment.INTRA_SECTOR_SECTOR){
                ...
                ctrlParam[profCnt] = {
                  ...
                  "secTypeId": secType,
                  "lblName": lblName,
                   "fldVal": frmConfgId==973 ? 0 :elmVal,
                    "ctrlDtls": ctrlDtls,
    ...
    var capacity= $("input.capacity[type='text']").val();
    var unittype = document.getElementsByClassName("unit")[1].getAttribute('id');
    var unittypeval=$('#'+unittype).val();
    ...
    if(intMkuyProjectType==environment.MILLET_SERVICE_CENTER){
      var vchcinNumber = $("input.cinNumber[type='text']").val();
      var milletType = document.getElementsByClassName("milletType")[1].getAttribute('id');
      var vchmilletType=$('#'+milletType).val();
    }
    else{
       vchcinNumber='';
       vchmilletType='';
   ...
   if(intMkuyProjectSector=='6'){
     intMkuyProjectType=0;
   }
   if(intMkuyProjectSector!='6' && intMkuyProjectType=='0'){
    Swal.fire({
      icon: 'error',
  ...
  if((intMkuyProjectType!='86' && intMkuyProjectSector!='6') || (intMkuyProjectSector!='6' && intMkuyProjectSector!='5')){
    if(capacity==''){
      Swal.fire({
        icon: 'error',
        text: 'Enter Capacity'
    ...
    if(capacity=='-'){
      Swal.fire({
        icon: 'error',
        text: 'Please enter value after negative symbol for capacity'
      });
      return false;
    }
    if(unittypeval==''){
    ...
    var workflow = document.getElementsByClassName("work_flow_id")[1].getAttribute('id');
    var workflowId=$('#'+workflow).val();
    var fixed_costId = document.getElementsByClassName("fixed_cost")[1].getAttribute('id');
    var fixed_cost_val:any =$('#'+fixed_costId).val();
    fixed_cost_val = fixed_cost_val!=''?parseInt(fixed_cost_val):0;
    var project_costId = document.getElementsByClassName("project_cost")[1].getAttribute('id');
    var project_cost_val:any = $('#'+project_costId).val();
    project_cost_val = project_cost_val!=''?parseInt(project_cost_val):0;

    var hypen_id = document.getElementsByClassName("hypen")[1].getAttribute('id');
    var hypen=$('#'+hypen_id).val();
    var hypenl=$('#'+hypen_id).attr('data-lblname');
    if(!this.vldChkLst.onlyhypen(hypen,hypenl)){
      return false
    }
    if(fixed_cost_val <= 0){
        Swal.fire({
          icon: 'error',
          text: 'Fixed Cost must be greater than zero.'
        });
        return false;
    }
    if(project_cost_val <= 0){
          Swal.fire({
          icon: 'error',
          text: 'Project Cost must be greater than zero.'
        });
        return false;
    }
    if(fixed_cost_val > project_cost_val){
          Swal.fire({
          icon: 'error',
          text: 'Project Cost must be greater than Fixed Cost.'
        });
        return false;
    }
    // $('.pos_itive').each(function(){
    //   let pos_itive_id = $(this).attr('id');
    //   let pos_itive_val=Number($("#"+pos_itive_id).val());
    //   if(pos_itive_val<0){
    //    Swal.fire({
    //       icon: 'error',
    //       text: 'Area value can`t be in negative'
    //     });
    //     return false;
    //   }
    // });
    ...
    if (valSts) {
      ...
      this.loading = true;
        let schemeParam = {
          ...
          'vchcinNumber':vchcinNumber,
          'vchmilletType':vchmilletType,
          ...
          if (res.status == 1) {
            if (this.isDraft) {
              ...
              if (this.docSectnSts) {
                ...
                if(workflowId=='Mirror'){
                  this.financeType=true;
                  let encAppCtnId = this.encDec.encText((this.schemeId + ':' + res.appCtnId + ':' + this.apprRsmSts+ ':' + this.financeType + ':' + this.aadhaarNo + ':' + this.onlineProfileId).toString());
  ...
  keyPressEvt(type: any, evt: any, crtId: any) {
    ...
    //  sts = this.vldChkLst.isAlphaNumeric(evt);
      ...
      if (type == 4) {
        if (classnameArr.includes("pos_itive")){
          if(evt.charCode==45){
  ...
  keyUpEvt(type: any, evt: any, crtId: any) {
    ...
    for(let cal in this.jsnFormulaCalculation){
      ...
      if(calArr.includes(crtId)){
        ...
        $('#'+ cal).parents('.form-group').append('<small class="d-block mt-1">'+this.get_price_to_word((<HTMLInputElement>document.getElementById(cal)).value)+'</small>');
 ...
 restrictMinusSymbol(event) {
    // Check if the pressed key is the minus symbol (-)
    if (event.key === '-') {
        // Prevent the default action (typing the minus symbol)
        event.preventDefault();
    }
}
  ...
  addRow(adMrSctn: any, adMrRow: any, adMrCol: any, adMrArr: any,cls:any) {
    ...
    // if(cls.includes('bhulekh_readonly')){
    //   $('.pos_itive').each(function(){
    //     let pos_itive_id = $(this).attr('id');
    //     let pos_itive_val=Number($("#"+pos_itive_id).val());
    //     if(pos_itive_val<0){
    ...
    let curObj1 = this;
    setTimeout(function () {
      curObj1.bhulekhaInfo(adMrRow);
    }, 0);
  }
  async bhulekhaInfo(adMrRow:any){
    let txtval = $("input[type='radio'].non_bhulekh_land_details:checked").attr("id");
    let landval = $("label[for='" + txtval + "']").text();
    if ($.trim(landval) == "Yes") {
      $('.bhulekh_readonly').prop('readonly', true);
      $("[id^='btnArea']" ).show();
    ...
    if($.trim(landval) == "No") {
      $('.bhulekh_readonly').removeAttr('readonly');
      $("[id^='btnArea']" ).hide();
    }
  }
...
// this.schemeForm.value['addMore1678794448914_2_8']=[];
// this.schemeForm.value['addMore1678186029978_2_9']=[];
    if(classVal.includes('mkuy_project')){
      ...
      if(parseInt(val)==environment.APICOL_CAGE_CULTURE){
        ...
        $('.clsDiv_addMore1678794448914_2_8').find('.table tbody tr td').children('input').val('');
       // console.log(this.schemeForm.controls['addMore1678794448914_2_8']);
      //  this.schemeForm.removeControl('addMore1678794448914_2_8');
     // this.schemeForm.controls['addMore1678794448914_2_8'].setValue([]);
        // $('.clsDiv_addMore1678794448914_2_8').find('.table').remove();
        // this.schemeForm.value['addMore1678794448914_2_8'].length=0;
        // console.log('ii');
        $("#"+environment.INTER_SECTOR_SECTOR_INFO).css('display','block');
       // $(".product").css('display','none');
        $(".capacity").css('display','none');
        $(".unit").css('display','none');
      //  $("input.product[type=text]").val('0');
        $("input.capacity[type=text]").val('0');
        $(".unit").val(0);
        $(".milletService").css('display','none');
        $('.milletService').parent().hide();
        $("input.cinNumber[type=text]").val('0');
      ...
      else if(parseInt(val)==environment.MILLET_SERVICE_CENTER){
...
//Intra Sector Details
      if(classVal.includes('sector_of_project')){
        $("#"+environment.INTER_SECTOR_SECTOR_INFO).css('display','none');
          ...
          //this.schemeForm.value['addMore1678186029978_2_9'].length=0;
          //$('.clsDiv_addMore1678186029978_2_9').find('.table tbody tr td').children('input').val('');
         // this.schemeForm.controls['addMore1678186029978_2_9'][0].setValue([]);
          //$('.clsDiv_addMore1678186029978_2_9').find('.table').remove();
          // $('.clsDiv_addMore1678186029978_2_9').find('.table tbody').children('tr:not(:first)').remove();
           // $(".product").css('display','none');
            $(".capacity").css('display','none');
            $(".unit").css('display','none');
            $(".mkuy_project").css('display','none');
            $(".mkuy_project").parent('div').css('display','none');
            //$("input.product[type=text]").val('0');
            $("input.capacity[type=text]").val(0);
            $(".unit").val(0);
             $("select.mkuy_project").val(0);
           ...
           // $(".product").css('display','block');
            ...
            $(".unit").css('display','block');
            $(".mkuy_project").css('display','block');
            $(".mkuy_project").parent('div').css('display','block');
            $("select.mkuy_project").val(0);
            // let mkuyproject_field: any = document.getElementsByClassName("select.mkuy_project");
            // let mkuyproject_field_id = mkuyproject_field.getAttribute('id')
            // this.schemeForm.patchValue({ [mkuyproject_field_id]: 0 });
    ...
    if(classVal.includes('intra_sector_details')){
      $('.clsDiv_addMore1678794448914_2_8').find('.table tbody tr td').children('input').val('');
      $('.clsDiv_addMore1678794448914_2_8').find('.table tbody').children('tr:not(:first)').remove();
      let idAttr = e.target.attributes.id;
      let id = idAttr.nodeValue;
      let val = (<HTMLInputElement>document.getElementById(id)).value;
    ...
    // if(classVal.includes('type_of_applicant')){
    //     let idAttr = e.target.attributes.id;
    //     let id = idAttr.nodeValue;
    //     let val = (<HTMLInputElement>document.getElementById(id)).value;
    //     if(val==environment.GROUP){
    //       $("#"+environment.GROUP_MEMBER).css('display','block');
    //       $("#"+environment.ENTER_PRISE_MEMBER).css('display','none');
    //     }
    //    if(val==environment.ENTERPRISE){
    //       $("#"+environment.ENTER_PRISE_MEMBER).css('display','block');
    //       $("#"+environment.GROUP_MEMBER).css('display','none');
    //     }
    //     if(val==environment.INDIVIDUAL){
    //       $("#"+environment.GROUP_MEMBER).css('display','none');
    //       $("#"+environment.ENTER_PRISE_MEMBER).css('display','none');
    //     }
    //   }
