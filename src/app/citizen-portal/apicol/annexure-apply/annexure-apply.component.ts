import { Component, OnInit, ElementRef, Injectable, ViewChild,Renderer2,AfterViewInit  } from '@angular/core';
...
@Component({
  selector: 'app-annexure-apply',
  templateUrl: './annexure-apply.component.html',
  styleUrls: ['./annexure-apply.component.css'],
  providers: [CitizenProfileService, ValidatorchklistService, NgbDateCustomParserFormatter]

})
export class AnnexureApplyComponent implements OnInit,AfterViewInit {
  loading = false;
  ...
  ngOnInit(): void {
    ...
    this.financeType = JSON.parse(sessionStorage.getItem('FINANCE_ANNEXURE'));
    ...
    console.log(schemeArr,'annexure-apply');
  ...
  getSchmDynmCtrls() {
    ...
    this.loading = true;
    this.objSchmCtrl.schemeAnnexureDetails(params).subscribe(res => {
  ...
  async docompleteAnnexure() {
    ...
    if (valSts) {
      ...
      for (let key of controlKeys) {
        ...
        let columnMnd = this.controlTypeArr[key]['columnMnd'];
        ...
        if (ctrlType == 8) {
          ...
          for (let adMrCtrlRw = 0; adMrCtrlRw < adMrArr.controls.length; ++adMrCtrlRw) {
            ...
            for (let adMrCtrlCl = 0; adMrCtrlCl < objCtrlKey.length; ++adMrCtrlCl) {
              ...
              if(addmoreFlagDep==1){
    // console.log(this.controlTypeArr[key].jsnControlArray[0].ctrlAddMore[adMrCtrlCl]);
                //  if($('#'+this.controlTypeArr[key]['vchLabelName']).css('display') == 'block'){
                  let adMrValParam = { "dynDataObj": this.controlTypeArr[key].jsnControlArray[0].ctrlAddMore[adMrCtrlCl], "ctrlVal": adMrCTrlVal, "dispNnSts": dispNnSts, "ctrlType": ctrlType,drftSts:this.isDraft };
        ...
        let ctrlDtls = '';
        let ctrlValParam = { "dynDataObj": this.controlTypeArr[key], "ctrlVal": elmVal, "dispNnSts": dispNnSts, "ctrlType": ctrlType,drftSts:this.isDraft };
    ...
    if (valSts) {
      ...
      let schemeParam = {
        ...
        "saveStatus":this.isDraft,
      }
      this.objSchmCtrl.completeAnnexure(schemeParam).subscribe(res => {
        if (res.status == 1) {
          if (this.isDraft) {
            this.loading = false;
            Swal.fire({
              icon: 'success',
              text: 'Drafted Succesfully'
            });
          }
          else {
  ...
  keyUpEvt(type: any, evt: any, crtId: any) {
    ...
    var waTotalN :any= 0;
    var mTotalN:any=0;
    var majorTotalN:any=0;
    var minAmountTotalN:any=0;
    var misTotalN:any=0;
    var fenchingCost:any=$("input:text.lFencing").val();
    var landCost:any=$("input:text.lPond").val();
    var OtherCost:any=$("input:text.lOther").val();
    var grandTotalId = document.getElementsByClassName('grandTotal')[0];
    var grandTotal_total_name = grandTotalId.getAttribute('name');
    var grandTotal=parseFloat(fenchingCost)+parseFloat(landCost)+parseFloat(OtherCost);
    var cAmount:any =0;
    var eAmount:any =0;
    var wAmount:any =0;
    var miAmount:any =0;
    ...
    var minAmount:any=0;
    var misAmount:any=0;
    if (cls.search("cAmount") > 0) {
      $("input:text.cAmount").each(function() {
        cAmount= $(this).val()?$(this).val():0;
        cAmount = isNaN(cAmount) ? 0 : cAmount;
       conTotalN = conTotalN + parseFloat(cAmount);
      }); 
      var conTotalId = document.getElementsByClassName('conTotal')[0];
      let con_total_name = conTotalId.getAttribute('name');
      this.schemeForm.patchValue({ [con_total_name]: conTotalN });
     
      $('.conTotal').val(conTotalN);
      //grandTotal=grandTotal+conTotalN;
     
    }
   
    if (cls.search("eAmount") > 0) {
    
      $("input:text.eAmount").each(function() {
        eAmount= $(this).val()?$(this).val():0;
        eAmount = isNaN(eAmount) ? 0 : eAmount;
        eleTotalN = eleTotalN + parseFloat(eAmount);
      }); 
      var elTotalId = document.getElementsByClassName('eleTotal')[0];
      let el_total_name = elTotalId.getAttribute('name');
      this.schemeForm.patchValue({ [el_total_name]: eleTotalN });
    
      $('.eleTotal').val(eleTotalN);
      //grandTotal=grandTotal+eleTotalN;
    
     
    }
   
    if (cls.search("wAmount") > 0) {
      $("input:text.wAmount").each(function() {
        wAmount = $(this).val()?$(this).val():0;
        wAmount = isNaN(wAmount) ? 0 : wAmount;
        waTotalN = waTotalN + parseFloat(wAmount);
      }); 
      var waTotalId = document.getElementsByClassName('waTotal')[0];
      let wa_total_name = waTotalId.getAttribute('name');
      this.schemeForm.patchValue({ [wa_total_name]: waTotalN });
      
      $('.waTotal').val(waTotalN);
      //grandTotal=grandTotal+waTotalN;
    }
    if (cls.search("miAmount") > 0) {
      $("input:text.miAmount").each(function() {
        miAmount = $(this).val()?$(this).val():0;
        miAmount = isNaN(miAmount) ? 0 : miAmount;
        mTotalN = mTotalN + parseFloat(miAmount);
      }); 
      var miTotalId = document.getElementsByClassName('miTotal')[0];
      let mi_total_name = miTotalId.getAttribute('name');
      this.schemeForm.patchValue({ [mi_total_name]: mTotalN });
     
      $('.miTotal').val(mTotalN);
      //grandTotal=grandTotal+mTotalN;
      
     
    }
    if (cls.search("mAmount") > 0) {
      $("input:text.mAmount").each(function() {
        mAmount = $(this).val()?$(this).val():0;
        mAmount = isNaN(mAmount) ? 0 : mAmount;
        majorTotalN = majorTotalN + parseFloat(mAmount);
      }); 
      var majorTotalId = document.getElementsByClassName('majorTotal')[0];
      let major_total_name = majorTotalId.getAttribute('name');
      this.schemeForm.patchValue({ [major_total_name]: majorTotalN });
    
      $('.majorTotal').val(majorTotalN);
      //grandTotal=grandTotal+majorTotalN;
      
    }
    if (cls.search("minAmount") > 0) {
      $("input:text.minAmount").each(function() {
         minAmount = $(this).val()?$(this).val():0;
         minAmount = isNaN(minAmount) ? 0 : minAmount;
        minAmountTotalN = minAmountTotalN + parseFloat(minAmount);
      }); 
      var minorTotalId = document.getElementsByClassName('mhnTotal')[0];
      let minor_total_name = minorTotalId.getAttribute('name');
      this.schemeForm.patchValue({ [minor_total_name]: minAmountTotalN });
     
      $('.mhnTotal').val(minAmountTotalN);
      //grandTotal=grandTotal+minAmountTotalN;
      
    }
    if (cls.search("misAmount") > 0) {
      $("input:text.misAmount").each(function() {
         misAmount = $(this).val()?$(this).val():0;
         misAmount = isNaN(misAmount) ? 0 : misAmount;
        misTotalN = misTotalN + parseFloat(misAmount);
      }); 
      var misTotalId = document.getElementsByClassName('misTotal')[0];
      let mis_total_name = misTotalId.getAttribute('name');
      this.schemeForm.patchValue({ [mis_total_name]: misTotalN });
      
      $('.misTotal').val(misTotalN);
      //grandTotal=grandTotal+misTotalN;
    }
    
   
    

    var conTotal:any= $('.conTotal').val()?$('.conTotal').val():0;
    conTotal = isNaN(conTotal) ? 0 : conTotal;
    var eleTotal:any= $('.eleTotal').val()?$('.eleTotal').val():0;
    eleTotal = isNaN(eleTotal) ? 0 : eleTotal;
    var waTotal:any= $('.waTotal').val()?$('.waTotal').val():0;
    waTotal = isNaN(waTotal) ? 0 : waTotal;
    var miTotal:any= $('.miTotal').val()?$('.miTotal').val():0;
    miTotal = isNaN(miTotal) ? 0 : miTotal;
    var majorTotal:any= $('.majorTotal').val()?$('.majorTotal').val():0;
    majorTotal = isNaN(majorTotal) ? 0 : majorTotal;
    var mhnTotal:any= $('.mhnTotal').val()?$('.mhnTotal').val():0;
    mhnTotal = isNaN(mhnTotal) ? 0 : mhnTotal;
    var misTotal:any= $('.misTotal').val()?$('.misTotal').val():0;
    misTotal = isNaN(misTotal) ? 0 : misTotal;
    grandTotal = grandTotal+parseFloat(conTotal)+parseFloat(eleTotal)+parseFloat(waTotal)+parseFloat(miTotal)+parseFloat(majorTotal)+parseFloat(mhnTotal)+parseFloat(misTotal);
    this.schemeForm.patchValue({ [grandTotal_total_name]: grandTotal });
  ...
  open(content: any) {
    ...
    this.modalService.open(content, { size: 'xl', backdrop: 'static', keyboard: false, ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
