...
export class UpdateschemestatusComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.schemeId!=44){
     ...
     // let encSchemeStr = this.encDec.encText(this.schemeStr.toString());
      thisObjnew.router.navigate(['citizen-portal/dashboard']);
    }
    ...
    this.schemeApi.getApplicationActivityStatus(params).subscribe(res => {
      ...
      this.bankname = res.NameofBank;
      this.branchname=res.NameofBranch;
      this.ifsccode=res.IFSCCode;
      this.loanamt=res.LoanAmount;
      this.bankid=res.BankId;
      this.appdistName=res.distName;
      this.appblockName=res.blockName;
      this.bankStatus=res.bankStatus;
      this.workflowId=res.intWorkflowEffectId;
      this.tinStatus=res.tinStatus;

      this.loanamt=res.LoanAmount;
      this.bankid=res.BankId;
      this.intAdvanceCisStatus=res.intAdvanceCisStatus;
  ...
  updateStatus() {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (taskId == '') {
        ...
        if (taskId == '2') {
          ...
          let progfileName = activityId + '_prog_file';
          let progselectedFileList = (<HTMLInputElement>document.getElementById(progfileName));
          let progfile: any = progselectedFileList.files.item(0);
          const progextension = progselectedFileList.value.split('.').pop();
          if(progfile){
            let fileType = 'png,jpg,jpeg';
            let fileFileSize = 1;
            let proguploadedFileType = progfile.type;
            let proguploadedFileSize = progfile.size;

            let UploadFileConvesion = Math.round((proguploadedFileSize / 1024));
            let acceptableTypes = fileType.split(',');
            const accepteableLowercase = acceptableTypes.map(acceptableTypes => acceptableTypes.toLowerCase());
            if (accepteableLowercase.includes(progextension.toLowerCase()) == false) {
              Swal.fire({
                icon: 'error',
                text: 'Upload only ' + fileType + ' files '
              });
              ...
              progfile = (progfile) ? progfile : '';
              formData.append('progfileName_' + profId, progfile);
              fileNames += progfile + ",";
            }
          }
          else {
    ...
    if (this.totalItem == this.completeItem) {
      ...
      if(this.editPhaseActivity==0){
        ...
        console.log(hdnintmnFileNameDoc);
        if(hdnintmnFileNameDoc=='' && intmnFile!=null){
            intmnFile = (intmnFile) ? intmnFile : '';
            formData.append('hdnintmnFile'+i, intmnFile);
            formData.append('label'+i, intmnLabelName);
        }

        else if(intmnFile==null && hdnintmnFileNameDoc!=''){
          hdnintmnFileNameDoc = (hdnintmnFileNameDoc) ? hdnintmnFileNameDoc : '';
          formData.append('hdnintmnFile'+i, hdnintmnFileNameDoc);
          formData.append('label'+i, intmnLabelName);
        }
      else if (this.completionDocument[i].tinIsMandatory==1) {
        if(intmnFile){
      ...
      if(this.completionDocument[i].tinIsMandatory==1){
      if (intmnFile) {
        let intmnFileType = 'jpeg,jpg,gif,pdf,zip';
        let intmnFileSize = 5;
        let uploadedIntmnFileSize = intmnFile.size;
    ...
    formData.append('nofodoc', this.completionDocument.length);
    ...
    for (let item of this.lastUpd) {
      ...
      if (item.farmerActionStatus != null) {
        ...
        if (item.farmerActionStatus == 1) {
          const ele1 = document.getElementById(item.activityActnId + "_compl") as HTMLInputElement;
          if(this.workflowId==1){
            if(this.bankid>0){
              if(this.bankStatus!=45){
                ...
                Swal.fire({
                  title: 'Notification!',
                  icon: "info",
                  text: "After Bank loan you can give complete completion"
                });
              }
              else{
                this.isCompleted="";
            ...
            if(this.bankid>0){
              if(this.intAdvanceCisStatus!=1){
                
                ele1.disabled = true;
                Swal.fire({
                  title: 'Notification!',
                  icon: "info",
                  text: "After Upfront subsidy release you can give complete completion"
                });
              }
              else{
                this.isCompleted="";
        ...
        if(this.bankid>0){
          if(this.bankStatus==45){
            ...
            const ele1 = document.getElementById(item.activityActnId + "_inprog") as HTMLInputElement;
            this.updateForm.patchValue({ [actId]: 1 });
            ele1.checked = true;
            ele1.click();
            ele.disabled = true;
            Swal.fire({
  ...
  doCompletion() {
    ...
    if (valSts) {
      ...
      for (let key of controlKeys) {
        ...
        let ctrlDtls = '';
        let ctrlValParam = { "dynDataObj": this.controlTypeArr[key], "ctrlVal": elmVal, "dispNnSts": dispNnSts, "ctrlType": ctrlType,'drftSts':false };
  ...
  getApplicationData() {
    ...
    this.open(this.apponlineCompletionModalRef);
    let curObj = this;
     setTimeout(function () {
      curObj.sethierarchyDetails();
    }, 1000);
    if(this.bankid>0){
     let curObj = this;
     setTimeout(function () {
      curObj.setBankDetails();
    }, 1000);
    }
    else{
      let curObj = this;
      setTimeout(function () {
       curObj.hideBankDetails();
     }, 1000);
    }
  }
  setBankDetails(){
    let loan_amt_field: any = document.getElementsByClassName("loanamt")[0];
    let loan_amt_id = loan_amt_field.getAttribute('id');
    this.schemeForm.patchValue({[loan_amt_id]:this.loanamt});
    let loan_bank_field: any = document.getElementsByClassName("loan_bank_name")[0];
    let loan_bank_id = loan_bank_field.getAttribute('id');
    this.schemeForm.patchValue({[loan_bank_id]:this.bankname});
    let loan_branch_field: any = document.getElementsByClassName("loan_bank_branch")[0];
    let loan_branch_id = loan_branch_field.getAttribute('id');
    this.schemeForm.patchValue({[loan_branch_id]:this.branchname});
    let loan_ifsc_field: any = document.getElementsByClassName("ifsc_loan_code")[0];
    let loan_ifsc_id = loan_ifsc_field.getAttribute('id');
    this.schemeForm.patchValue({[loan_ifsc_id]:this.ifsccode});
    let mean_of_finance: any = document.getElementsByClassName("mean_of_finance")[0];
    let mean_of_finance_id = mean_of_finance.getAttribute('id');
    this.schemeForm.patchValue({[mean_of_finance_id]:'Bank Loan'});
    $('.mean_of_finance').attr('readonly','true');
    $('.loanamt').attr('readonly','true');
  }
  hideBankDetails(){
    let loan_amt_field: any = document.getElementsByClassName("loanamt")[0];
    let loan_amt_id = loan_amt_field.getAttribute('id');
    $('.clsDiv_'+loan_amt_id).css('display','none');
    let loan_bank_field: any = document.getElementsByClassName("loan_bank_name")[0];
    let loan_bank_id = loan_bank_field.getAttribute('id');
    $('.clsDiv_'+loan_bank_id).css('display','none');
    let loan_branch_field: any = document.getElementsByClassName("loan_bank_branch")[0];
    let loan_branch_id = loan_branch_field.getAttribute('id');
    $('.clsDiv_'+loan_branch_id).css('display','none');
    let loan_ifsc_field: any = document.getElementsByClassName("ifsc_loan_code")[0];
    let loan_ifsc_id = loan_ifsc_field.getAttribute('id');
    $('.clsDiv_'+loan_ifsc_id).css('display','none');
    let loan_acc_field: any = document.getElementsByClassName("loanaccno")[0];
    let loan_acc_id = loan_acc_field.getAttribute('id');
    $('.clsDiv_'+loan_acc_id).css('display','none');
    let mean_of_finance: any = document.getElementsByClassName("mean_of_finance")[0];
    let mean_of_finance_id = mean_of_finance.getAttribute('id');
    this.schemeForm.patchValue({[mean_of_finance_id]:'Self'});
    $('.mean_of_finance').attr('readonly','true');
  ...
  sethierarchyDetails(){
    ...
    let distdetails: any = document.getElementsByClassName("distName")[0];
    let distid = distdetails.getAttribute('id');
    this.schemeForm.patchValue({[distid]:this.appdistName});
    $('.distName').attr('readonly','true');

    let blockdetails: any = document.getElementsByClassName("blockName")[0];
    let blockid = blockdetails.getAttribute('id');
    this.schemeForm.patchValue({[blockid]:this.appblockName});
