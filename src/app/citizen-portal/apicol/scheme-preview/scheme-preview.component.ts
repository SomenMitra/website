...
export class SchemePreviewComponent implements OnInit {
  ...
  console.log(schemeArr,'scheme-previwdocumnt');
    this.schemeId = schemeArr[0];
    let APICOL_SCHEME_IDS = environment.APICOL_SCHEME_IDS;
    if(APICOL_SCHEME_IDS.includes(this.schemeId)){
      this.router.navigate(['/citizen-portal/apicol/scheme-preview', encSchemeId]);
    }
    this.applctnId = schemeArr[1];
    
    this.financeType= schemeArr[3]==='true'?true:false;
  ...
  getFarmrInfo() {
    ...
    this.objProf.profileBuild(params).subscribe(res => {
      ...
      if (res.status > 0) {
        ...
        this.modeofFinance = resProfInfo['vchmeanoffinance'];
       
      if(this.modeofFinance=='Bank Loan'){
  ...
  getSchmDynmCtrls() {
    ...
    this.objSchmCtrl.schemeAnnexureDetails(params).subscribe(res => {
      if (res.status > 0) {
        ...
        this.schmSts = res.status;
        this.schmDataAn = res.result['ctrlArr'];
        this.schmHrDtAn = res.result['schmSrvArr'];
        this.intApplicationStatus = this.schmHrDt.intApplicationStatus;

        for (let sectnInfo in this.schmDataAn) {
          for (let ctrlInfo in this.schmDataAn[sectnInfo]['sectionCtrls']) {
            let ctrlArr = this.schmDataAn[sectnInfo]['sectionCtrls'][ctrlInfo];
            let ctrlClass = ctrlArr['jsnControlArray'][0]['ctrlClass'];
            if(ctrlClass=='Self_Finance'){
              this.means_finance = ctrlArr['vchFieldValue'];
            }
  ...
  doSchmModify() {
    ...
    let encAppCtnId = this.encDec.encText((this.schemeId + ':' + this.applctnId + ':' + docSectnFlg)+ ':' + this.financeType+':'+this.aadhaarNoNew+':'+this.onlineProfileId.toString());
  ...
  async onFinalSubmit() {
    ...
    if(this.schemeId==environment.sujogPortal || this.schemeId==environment.seedDBT){
      ...
      this.objSchmCtrl.schemeFnlSubmitSujog(schemeParam).subscribe(res => {
        if (res.status == 1) {
          this.loading = false;
          let encAppCtnId = this.encDec.encText((this.schemeId + ':' + this.applctnId+':'+this.aadhaarNo+':'+this.onlineProfileId).toString());
          this.router.navigate(['/citizen-portal/apicol/success', encAppCtnId]);
        }
        else {
          Swal.fire({
            icon: 'error',
    ...
    if(this.paySectnSts &&  this.resPaySts ==1 && environment.PAY_ENABLE=='YES'){
      const validationResponse = await this.checkAadhaarValidation(this.aadhaarNoNew, this.onlineProfileId);
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.objSchmCtrl.schemePaySubmit(schemeParam).subscribe(res => {
            ...
            if (res.status == 1) {
              ...
              if(result.PAYMENT_CONFIG=='CC AVENUE'){
                this.sendCCPayment(result)
              }else{
...
<h5 class="mb-3 text-success">Voucher Details</h5>
  ...
  goBack() {
    //window.history.back();
    let encSchemeId = this.route.snapshot.paramMap.get('id');
    this.router.navigate(['/citizen-portal/apicol/scheme-applied', encSchemeId]);
  }
  ...
  getPaymentDetails(){
    ...
    this.objSchmCtrl.getSchmPayList(params).subscribe(res => {
      if (res.result.applctnSts > 0) {
        ...
        this.prevpayment= res.result['prevpayment'];
        this.payStatus= this.paymentDetails.vchErrorCode;
        this.paymenttime= this.prevpayment.stmCreatedDate;
        const minutesToAdjust = 30;
        const millisecondsPerMinute = 60000;
        const originalDate = new Date(this.paymenttime);
        const modifiedDate2 = new Date(originalDate.valueOf() + (minutesToAdjust * millisecondsPerMinute));

         this.myShortFormat = this.pipe.transform(modifiedDate2, 'dd-MM-yyyy hh:mm a');

        //console.log(myShortFormat);
        this.loading = false;
      }
    },
      error => {
  ...
  sendPayment(result:any){
    ...
    sendCCPayment(result:any){

      this.loading = true;
        var mapForm = document.createElement("form");
        mapForm.method = "POST"; // or "post" if appropriate
        mapForm.name = 'redirect';
        mapForm.action = result.action;
        var hdn_qs = document.createElement("input");
        hdn_qs.type = "hidden";
        hdn_qs.name = "encRequest";
        hdn_qs.setAttribute("value",result.encRequest);
        mapForm.appendChild(hdn_qs);

        var actionUrl = document.createElement("input");
        actionUrl.type = "hidden";
        actionUrl.name = "access_code";
        actionUrl.setAttribute("value", result.access_code);
        mapForm.appendChild(actionUrl);
