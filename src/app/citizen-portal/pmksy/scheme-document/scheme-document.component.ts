...
export class SchemeDocumentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let koSesnInfo  = JSON.parse(sessionStorage.getItem('USER_SESSION_KO_DATA'));    
    this.koApplicantName   = koSesnInfo.name;
        ...
        if (event.type === HttpEventType.UploadProgress) {
          ...
          if(upSts==1)
          {
            ...
            if(this.isDraft)
            {
              ...
              if (!$('#chkDocumentDeclr').is(':checked')) {
                Swal.fire({
                  icon: 'error',
                  text: 'Please check the declaration'
                });
                this.loading = false;
              }else{
