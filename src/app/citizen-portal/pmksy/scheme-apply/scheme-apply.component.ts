...
export class SchemeApplyComponent implements OnInit {
  ...
  getMI(compnentId){
    ...
    let applicationId=this.applctnId;
    let landParam = {
      "intProcessId": intProcessId,
      "componentId":compnentId,
      "language":localStorage.getItem('locale'),
      "onlineServiceId":applicationId
    };
    this.loading = true;
    this.objSchmCtrl.getMI(landParam).subscribe(res => {
      if(res.result['status']==200){
        this.loading = false;
        let chldArr=res.result['resultData'];
        let intManufactureId=res.result['intManufactureId'];
        let chldOptnArr='<option value="">Select</option>';
        for (let i = 0; i < chldArr.length; i++){
          let selected = (intManufactureId == chldArr[i].intProfileId) ? 'selected="selected"' : '';
          chldOptnArr += '<option value="' + chldArr[i].intProfileId + '" ' + selected + '>' + chldArr[i].vchApplicantName + '</option>';
        }
        let MIField=<HTMLElement>document.querySelector(".pmksy_mi_id ");
        MIField.innerHTML = chldOptnArr;//res.result['result'];
      }
    });
  }
      ...
      if (res.status > 0) {
        ...
        for (let sectnInfo in this.respDynm) {
          for (let ctrlInfo in this.respDynm[sectnInfo]['sectionCtrls']) {
            ...
            if (ctrlClass == 'pmksy_component') {
              this.compo = ctrlNm;
            }
            if (ctrlClass == 'pmksy_sub_component') {
              this.Subcompo = ctrlNm;
            }
            if (ctrlClass == 'cls_crop_type') {
              this.cropType = ctrlNm;
            }
            if (ctrlClass == 'cls_crop_name') {
              this.cropName = ctrlNm;
            }
        ...
        setTimeout(function () {
          ...
          curObj.aadhaarWiseVerification();
        }, 1000);
        this.loading = false;
      }
    });
  }
  aadhaarWiseVerification(){  
    if(this.vchAadhaarNo){
      let paramArr={
        'aadhaarNo':this.vchAadhaarNo
      }
      this.objPMKSY.getAadhaarWiseAreaDetails(paramArr).subscribe(res => {
        if(res.status==1 && res.result.resultInfo.koExist==1){
          this.koExistAadhaarWise=res.result.resultInfo.khataPlotDetls;
        }        
      });
    }
  }
  setDynamicVal() {
    ...
    //   }
    // }
    if (this.compo != '' && this.compo != 'undefined' && this.applctnId > 0) {
      let compoId:any = this.dynElement[this.compo]["val"];
      setTimeout(()=>{                           
        this.getSubComponent(compoId);
        this.getMI(compoId);
      }, 3000);
      
    }
    if (this.cropType != '' && this.cropType != 'undefined' && this.applctnId > 0) {
      let cropType:any = this.dynElement[this.cropType]["val"];
      setTimeout(()=>{
        this.getCrop(cropType);
      }, 3000);
    }
    ...
    let accountholderrelation  = landInfo_data.accountholderrelation;
    let accountholderrelationId  = landInfo_data.accountholderrelationId;
    this.koLandLength  = landInfo_data.land_detail.length;
    
    if(accountholderrelation == 'Self' && accountholderrelationId == 5){
  ...
  getSubComponent(eVlue){
    let paramVal = {
      "parentId": eVlue,
      "onlineServiceId":this.applctnId
    }
    this.objPMKSY.getSubComponentEdit(paramVal).subscribe(res => {    
      let chldArr=res.result['getDatas'];
      let subCompId=res.result['subCompId'];
      let chldOptnArr='';
      //if(subCompId>0){
        for (let i = 0; i < chldArr.length; i++){
          let selected = (subCompId == chldArr[i].intSubcomponentId) ? 'selected="selected"' : '';
          chldOptnArr += '<option value="' + chldArr[i].intSubcomponentId + '" ' + selected + '>' + chldArr[i].subcomponentName + '</option>';
        }
      //}
      
      document.querySelector('.pmksy_sub_component').innerHTML = chldOptnArr;
    });
  }
  getCrop(eVlue){
    let paramVal = {
    ...
    this.objPMKSY.getCropEdit(paramVal).subscribe(res => {    
      let chldArr=res.result['getDatas'];
      let cropEditId=res.result['cropId'];
      let chldOptnArr='<option value="">Select</option>';
      for (let i = 0; i < chldArr.length; i++){
        let selected = (cropEditId == chldArr[i].intCropId) ? 'selected="selected"' : '';
        chldOptnArr += '<option value="' + chldArr[i].intCropId + '" ' + selected + '>' + chldArr[i].vchCropName + '</option>';
      }
      document.querySelector('.cls_crop_name').innerHTML = chldOptnArr;
      let cropNameId=$(".cls_txt_cropName").attr("id");
        let cropId:any;
        cropId=cropEditId//$(".cls_crop_name").val();
        if(environment.CROP_OTHER.includes(cropId) ){
          document.getElementById(cropNameId).hidden = false;
          $('label[for="' + cropNameId + '"]').show();
          $('.clsDiv_'+ cropNameId).show();
...
//className.includes('ifsc_ko_code') className.includes('ifsc_ko_bank') className.includes('ifsc_ko_branch') className.includes('ifsc_ko_account') className.includes('ifsc_ko_holder')
        ...
        if (res.status == 1) {
          if (this.isDraft) {
            ...
            if (this.docSectnSts) {
              ...
              this.router.navigate(['/citizen-portal/pmksy/scheme-document', encAppCtnId]);
            }
            else {
              let encAppCtnId = this.encDec.encText((this.schemeId + ':' + this.applctnId + ':1').toString());
              this.router.navigate(['/citizen-portal/pmksy/scheme-preview', encAppCtnId]);
            }
          }
        }
        else {
  ...
  goToBack() {
    ...
    this.router.navigate(['/citizen-portal/pmksy/profile-update', encAppCtnId]);
  }
  ...
  getKOArea(adRowCtr:any){    
    ...
    //this.koExistAadhaarWise
    this.open(this.someKOModalRef);

  }
  isSelected(khata_no: string, plot_no: string): boolean {
    const result = this.koExistAadhaarWise.some(
      land => land.Khata_No === khata_no && land.Plot_No === plot_no
    );
    return result;
  }
  ...
  searchSubsidy() {
    ...
    }if(landTypeVal == 'FRA Land'){      
      $('.fra_more_area').each(function(){
      let fra_acer = $(this).attr('id');
          let acerVal: any=(<HTMLInputElement>document.getElementById(fra_acer)).value;
          totAcerFra=parseFloat(totAcerFra) + parseFloat(acerVal);   
      });
    }  
    let totAcer=totAcerKO + totAcerLease + totAcerFra;
