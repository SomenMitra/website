...
export class UpdateschemestatusComponent implements OnInit {
  ...
  preImageCount: any;
  postImageCount: any;
  preVerificationSectnSts: boolean;
  postVerificationSectnSts: boolean;
  imageSrc: string;
  showModal: boolean;
  type: number;
  latitude: string;
  ...
  constructor(private router: Router,
    ...
    private schemeApi: ManufactureSchemeService,
  ...
  ngOnInit(): void {
    ...
    this.applicantId = schemeArr[4];
    this.editPhaseActivity = schemeArr[5];
    this.tinAppStatus = schemeArr[6];

    //this.schemeStrRedirect=this.encDec.encText(this.schemeId+':'+this.applicationId+':'+this.applicantId);
    ...
    this.schemeApi.getManufactureApplicationActivityStatus(params).subscribe(res => {
      ...
      //console.log(this.completionDocument);
  ...
  updateActivityStatus() {
    ...
    formData.append('tinAppStatus', this.tinAppStatus);
  ...
  updateStatus() {
    ...
    this.updateDataOld(formData);
  }

  // to update data in server
  updateData(formDataObj: FormData) {
    this.loading = true;
    this.schemeApi.updateManufactureSchmActivitySts(formDataObj).subscribe(res => {
      let upSts = res.status;
      let upMsg = res.msg;
      // start after uploaded
      if (upSts == 1) {
        this.loading = false;
        let encSchemeStr = this.encDec.encText(this.schemeId+':'+this.applicationId+':'+this.applicantId.toString());
        this.loading = false;
        var thisObj = this;
          Swal.fire({
            icon: 'success',
            ...
            thisObj.router.navigate(['/manufacture-portal/scheme-applied', encSchemeStr]);
          });
  ...
  //not pre post
  updateDataOld(formDataObj: FormData) {
    this.loading = true;
    this.schemeApi.updateMSchmActivitySts(formDataObj).subscribe(res => {
      let upSts = res.status;
      let upMsg = res.msg;
      // start after uploaded
      if (upSts == 1) {
  ...
  // openModal(imageSrc: string, longitude: string, latitude: string,type:number) {
