...
lang='';
  ...
  miReg: any;
  appStatus: any;
  miReapplyDate: string = '';
  showReapplyLink: boolean = false;
  miExpiryDate: any;
  ...
  if (localStorage.getItem('theme') === 'theme-dark') {
    ...
    if(this.loginSts)
    {
      ...
      this.miReg = farmerInfo.IS_MIREG;
      this.appStatus = farmerInfo.APPLICATION_STATUS;
      ...
      this.miReapplyDate = farmerInfo.MI_REAPPLY_DATE ? farmerInfo.MI_REAPPLY_DATE : '';
      this.miExpiryDate = farmerInfo.MI_EXPIRY_DATE ? farmerInfo.MI_EXPIRY_DATE : '';
        if(this.miReapplyDate) {
        const currentDate = new Date();
        const reapplyDate = new Date(this.miReapplyDate.split('/').reverse().join('-'));
          if (currentDate >= reapplyDate && this.miExpiryDate!='') {
          this.showReapplyLink = true;
        }
      }
    }
